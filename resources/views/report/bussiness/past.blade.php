        ...
        <div class="content-box">
            <div class="element-wrapper">
                <div class="element-box">
                    <div class="row">
                        <div class="col-12">
                            <form method="POST" action="/report/bussiness/past/table" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6">
                    ...
                    <div class="table-responsive">
                        <table id="myTable" class="table table-striped table-lightfont">
                            <thead>
                                ...
                                <tr>
                                    <th>{{ trans('app.lastyearthismonth') }}</th>
                                    <th>{{ trans('app.thismonth') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if(!empty($reports))
                                    ...
                                    @foreach ($reports as $item)
                                        ...
                                        <tr id="{{ generateMfo($item->mfo_id) }}">
                                            ...
                                            <td style="text-align: center">{{ (count((array) $array_filter) > 1)?(!empty($data->lastthismonth)?number_format((($data->thismonth/$data->lastthismonth)*100), 2, '.', ','):0):0 }}</td>
                                            <td style="text-align: center">{{ (count((array) $array_filter) > 1)?number_format(($data->lastthismonth), 0, '.', ','):0 }}</td>
                                            <td style="text-align: center">{{ (count((array) $array_filter) > 1)?number_format(($data->thismonth), 0, '.', ','):0 }}</td>
                                        </tr>
                                        @php
                                            $i++;
                                        @endphp
