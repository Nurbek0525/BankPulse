        ...
        <div class="content-box">
            <div class="element-wrapper">
                <div class="element-box">
                    <div class="row">
                        <div class="col-12">
                            <form method="POST" action="/report/business/guarantee/table" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6">
                        ...
                        <table id="myTable" class="table table-striped table-lightfont">
                            <thead>
                                <tr>
                                    ...
                                    <th rowspan="2" width='4%'class="color-black">{{ trans('app.mfo id') }}</th>
                                    <th rowspan="2" width='20%'class="color-black">{{ trans('app.average rating of banks') }}</th>
                                </tr>
                                <tr>
                                    <th class="color-black">{{ trans('app.no credit') }}</th>
                                    <th class="color-black">{{ trans('app.three credit') }}</th>
                                    <th class="color-black">{{ trans('app.six credit') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if(!empty($reports))
                                    ...
                                    @foreach ($reports as $item)
                                        @php
                                            $data = json_decode($item->b_guarantee);
                                        ...
                                        <tr id="{{ generateMfo($item->mfo_id) }}">
                                            ...
                                            <td style="text-align: center">{{ (count((array) $array_filter) > 1)?number_format($data->exist_case - ($data->nocredit + $data->threecredit + $data->sixcredit), 2, '.', ','):0 }}</td>
                                            <td style="text-align: center">{{ (count((array) $array_filter) > 1)?number_format(($data->nocredit), 0, '.', ','):'' }}</td>
                                            <td class="amount">{{ (count((array) $array_filter) > 1)?number_format(($data->threecredit), 0, '.', ','):'' }}</td>
                                            <td class="amount">{{ (count((array) $array_filter) > 1)?number_format(($data->sixcredit), 0, '.', ','):'' }}</td>
                                        </tr>
                                        @php
                                            $i++;
                                        @endphp
