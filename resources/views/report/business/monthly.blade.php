        ...
        <div class="content-box">
            <div class="element-wrapper">
                <div class="element-box">
                    <div class="row">
                        <div class="col-12">
                            <form method="POST" action="/report/business/monthly/table" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6">
                    ...
                    <div class="table-responsive">
                        <table id="myTable" class="table table-striped table-lightfont">
                            <thead>
                                <tr>
                                    <th rowspan="2" width='5%'class="color-black">#</th>
                            ...
                            <tbody>
                                @if(!empty($reports))
                                    ...
                                    @foreach ($reports as $item)
                                        @php
                                            $data = json_decode($item->b_m_report);
                                            $array_filter = (object) array_filter((array) $data, 'strlen');
                                            if(count((array) $array_filter) > 1){
                                                $exist_case = $data->exist_case??0;
                                                $poor_quality = $data->poor_quality??0;
                                                $delayed = $data->delayed??0;
                                                $not_provided = $data->not_provided??0;
                                                $rate = ($exist_case - ($poor_quality+$delayed+$not_provided))*($weight->b_m_report/100);
                                            }else{
                                                $exist_case = 0;
                                                $poor_quality = 0;
                                                $delayed = 0;
