        ...
        <div class="content-box">
            <div class="element-wrapper">
                <div class="element-box">
                    <div class="row">
                        <div class="col-12">
                            <form method="POST" action="/report/inspeksiya/likvidcredit/table" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6">
                        ...
                        <table id="myTable" class="table table-striped table-lightfont">
                            <thead>
                                ...
                                <tr>
                                    ...
                                    <th class="color-black">{{ trans('app.all credit') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if(!empty($reports))
                                    ...
                                    @foreach ($reports as $item)
                                        @php
                                            $data = json_decode($item->i_likvid_credit);
                                            if(!empty($data)){
                                                $exist_case = $data->exist_case;
                                                $alldeposit = $data->alldeposit;
                                                $allcredit = $data->allcredit;
                                                $rate = 100 - ((empty($allcredit))?0:(($alldeposit/$allcredit)*100 - $exist_case));
                                            }else{
                                                $alldeposit = 0;
                                                $allcredit = 0;
                                                $rate = 0;
                                        ...
                                        <tr id="{{ generateMfo($item->mfo_id) }}">
                                            ...
                                            <td style="text-align: center">{{ number_format($rate*($weight->i_likvid_credit/100), 2, '.', ' ') }}</td>
