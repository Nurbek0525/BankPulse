        ...
        <div class="content-box">
            <div class="element-wrapper">
                <div class="element-box">
                    <div class="row">
                        <div class="col-12">
                            <form method="POST" action="/report/inspeksiya/activelikvid/table" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6">
                    ...
                    <div class="table-responsive">
                        <table id="myTable" class="table table-striped table-lightfont">
                            <thead>
                                ...
                                <tr>
                                    <th class="color-black">{{ trans('app.active likvid') }}</th>
                            ...
                            <tbody>
                                @if(!empty($reports))
                                    ...
                                    @foreach ($reports as $item)
                                        @php
                                            $data = json_decode($item->i_active_likvid);
                                            if(!empty($data)){
                                                $exist_case = $data->exist_case;
                                                $active_likvids = $data->active_likvids;
                                                $allactive = $data->allactive;
                                                $rate = ((empty($allactive))?0:((intval($active_likvids)/intval($allactive))*$exist_case))*($weight->i_active_likvid);
                                        ...
                                        <tr id="{{ generateMfo($item->mfo_id) }}">
                                            ...
                                            <td class="amount">{{ (!empty($active_likvids))?number_format(($active_likvids), 0, '.', ' '):0 }}</td>
