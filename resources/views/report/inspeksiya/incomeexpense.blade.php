        ...
        <div class="content-box">
            <div class="element-wrapper">
                <div class="element-box">
                    <div class="row">
                        <div class="col-12">
                            <form method="POST" action="/report/inspeksiya/incomeexpense/table" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6">
                    ...
                    <div class="table-responsive">
                        <table id="myTable" class="table table-striped table-lightfont">
                            <thead>
                                ...
                                <tr>
                                    <th class="color-black">{{ trans('app.average income') }}</th>
                                    <th class="color-black">{{ trans('app.average expense') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if(!empty($reports))
                                    ...
                                    @foreach ($reports as $item)
                                        @php
                                            $data = json_decode($item->i_income_expense);
                                            if(!empty($data)){
                                                $exist_case = $data->exist_case;
                                                $averexpense = $data->averexpense;
                                                $averincome = $data->averincome;
                                                $i_i_e_percent = (empty($averincome))?0:(100 - ((($averexpense/$averincome)*100) - $exist_case));
                                                $rate = ($i_i_e_percent < 0)?0:$i_i_e_percent;
                                            }else{
                                                $averexpense = 0;
                                                $averincome = 0;
                                                $rate = 0;
                                        ...
                                        <tr id="{{ generateMfo($item->mfo_id) }}">
                                            ...
                                            <td style="text-align: center">{{ number_format($rate*($weight->i_income_expense/100), 2, '.', ' ') }}</td>
                                            <td class="amount">{{ (!empty($averincome))?number_format(($averincome), 0, '.', ' '):0 }}</td>
                                            <td class="amount">{{ (!empty($averexpense))?number_format(($averexpense), 0, '.', ' '):0 }}</td>
                                        </tr>
                                        @php
                                            $i++;
                                        @endphp
