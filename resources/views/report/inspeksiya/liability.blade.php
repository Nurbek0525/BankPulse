        ...
        <div class="content-box">
            <div class="element-wrapper">
                <div class="element-box">
                    <div class="row">
                        <div class="col-12">
                            <form method="POST" action="/report/inspeksiya/liability/table" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6">
                        ...
                        <table id="myTable" class="table table-striped table-lightfont">
                            <thead>
                                ...
                                <tr>
                                    ...
                                    <th class="color-black">{{ trans('app.people deposits') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if(!empty($reports))
                                    ...
                                    @foreach ($reports as $item)
                                        @php
                                            $data = json_decode($item->i_b_liability);
                                            if(!empty($data)){
                                                $exist_case = $data->exist_case;
                                                $peopledeposit = $data->peopledeposit;
                                                $alldeposit = $data->alldeposit;
                                                $rate = (empty($exist_case))?0:((empty($alldeposit))?0:((($peopledeposit/$alldeposit)*10000)/$exist_case));
                                            }else{
                                                $peopledeposit = 0;
                                                $alldeposit = 0;
                                                $rate = 0;
                                        ...
                                        <tr id="{{ generateMfo($item->mfo_id) }}">
                                            ...
                                            <td style="text-align: center">{{ number_format($rate*($weight->i_b_liability/100), 2, '.', ' ') }}</td>
                                            <td class="amount">{{ (!empty($alldeposit))?number_format(($alldeposit), 0, '.', ' '):0 }}</td>
                                            <td class="amount">{{ (!empty($peopledeposit))?number_format(($peopledeposit), 0, '.', ' '):0 }}</td>
                                        </tr>
                                        @php
                                            $i++;
                                        @endphp
