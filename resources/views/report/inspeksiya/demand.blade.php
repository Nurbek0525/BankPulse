        ...
        <div class="content-box">
            <div class="element-wrapper">
                <div class="element-box">
                    <div class="row">
                        <div class="col-12">
                            <form method="POST" action="/report/inspeksiya/demand/table" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6">
                        ...
                        <table id="myTable" class="table table-striped table-lightfont">
                            <thead>
                                ...
                                <tr>
                                    ...
                                    <th class="color-black">{{ trans('app.demand deposits') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if(!empty($reports))
                                    ...
                                    @foreach ($reports as $item)
                                        @php
                                            $data = json_decode($item->i_b_liability_demand);
                                            if(!empty($data)){
                                                $exist_case = $data->exist_case;
                                                $demanddeposit = $data->demanddeposit;
                                                $alldeposit = $data->alldeposit;
                                                $rate = (empty($alldeposit))?0:(100 - ((intval($demanddeposit)/intval($alldeposit))*100 - intval($exist_case)));
                                            }else{
                                                $demanddeposit = 0;
                                                $alldeposit = 0;
                                                $rate = 0;
                                        ...
                                        <tr id="{{ generateMfo($item->mfo_id) }}">
                                            ...
                                            <td style="text-align: center">{{ number_format($rate*($weight->i_b_liability_demand/100), 0, '.', ' ') }}</td>
                                            <td class="amount">{{ (!empty($alldeposit))?number_format(($alldeposit), 0, '.', ' '):0 }}</td>
                                            <td class="amount">{{ (!empty($demanddeposit))?number_format(($demanddeposit), 0, '.', ' '):0 }}</td>
                                        </tr>
                                        @php
                                            $i++;
                                        @endphp
