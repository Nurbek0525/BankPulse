        ...
        <div class="content-box">
            <div class="element-wrapper">
                <div class="element-box">
                    <div class="row">
                        <div class="col-12">
                            <form method="POST" action="/report/currency/check/table" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6">
                    ...
                    <div class="table-responsive">
                        <table id="myTable" class="table table-striped table-lightfont">
                            <thead>
                                ...
                                <tr>
                                    <th class="color-black">{{ trans('app.currency') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if(!empty($reports))
                                    ...
                                    @foreach ($reports as $item)
                                        @php
                                            $data = json_decode($item->c_phone);
                                            $percent = !empty($data->exist_case)?(($data->exist_case - $data->currency)/$data->exist_case)*100:0;
                                            $rate = $percent*$weight->c_phone;
                                        @endphp
                                        <tr id="{{ generateMfo($item->mfo_id) }}">
                                            ...
                                            <td style="text-align: center">{{ isset($data)?$data->currency:0 }}</td>
                                        </tr>
                                        @php
                                            $i++;
                                        @endphp
