        ...
        <div class="content-box">
            <div class="element-wrapper">
                <div class="element-box">
                    <div class="row">
                        <div class="col-12">
                            <form method="POST" action="/report/currency/execution/table" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6">
                            ...
                            <tbody>
                                @if(!empty($reports))
                                    ...
                                    @foreach ($reports as $item)
                                        @php
                                            $data = json_decode($item->c_execution);
                                            $percent = !empty($data->exist_case)?(($data->exist_case - ($data->not_provided + $data->poor_quality + $data->delayed))/$data->exist_case):0;
                                            $rate = $percent*$weight->c_execution;
                                        @endphp
                                        <tr id="{{ generateMfo($item->mfo_id) }}">
                                            ...
                                            <td style="text-align: center">{{ isset($data)?$data->poor_quality:0 }}</td>
                                            <td style="text-align: center">{{ isset($data)?$data->delayed:0 }}</td>
                                            <td style="text-align: center">{{ isset($data)?$data->not_provided:0 }}</td>
                                        </tr>
                                        @php
                                            $i++;
                                        @endphp
